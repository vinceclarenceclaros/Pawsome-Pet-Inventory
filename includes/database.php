...
public function executeUploadedSqlFile($uploadedFile) {
    // Get the contents of the uploaded SQL file
    $sql = file_get_contents($uploadedFile["tmp_name"]);
    // Execute the SQL query
    if ($this->con->multi_query($sql) === TRUE) {
        $msg = display_success_msg("SQL file executed successfully");
    } else {
        $msg = display_msg($this->con->error);
    }
    return $msg;
}
/*--------------------------------------------------------------*/
